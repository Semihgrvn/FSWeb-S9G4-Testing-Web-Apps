...
test('kullanıcı doğru ad ve soyad girdiğinde ama email girmediğinde BİR hata mesajı render ediliyor.', async () => {
    ...
    const adInput = screen.getByPlaceholderText("İlhan");
    userEvent.type(adInput, "abcasd");

    const soyadInput = screen.getByPlaceholderText("Mansız");
    userEvent.type(soyadInput, "abcasd");
...
test('geçersiz bir mail girildiğinde "email geçerli bir email adresi olmalıdır." hata mesajı render ediliyor', async () => {
    ...
    userEvent.type(emailInput, "abcasd");
...
test('ad,soyad, email render ediliyor. mesaj bölümü doldurulmadığında hata mesajı render edilmiyor.', async () => {
    ...
    const adInput = screen.getByPlaceholderText("İlhan");
    userEvent.type(adInput, "semih");

    const soyadInput = screen.getByPlaceholderText("Mansız");
    userEvent.type(soyadInput, "gervan");

    const emailInput = screen.getByLabelText(/Email*/i)
    userEvent.type(emailInput, "semo@gmail.com");

    const button = screen.getByText(/Gönder/i);
    userEvent.click(button);

    const ad = await screen.queryByText("semih");
    expect(ad).toBeInTheDocument();

    const soyad = await screen.queryByText("gervan");
    expect(soyad).toBeInTheDocument();

    const email = await screen.queryByText("semo@gmail.com");
...
test('form gönderildiğinde girilen tüm değerler render ediliyor.', async () => {
    ...
    userEvent.type(mail, "ornek@x.com");
    ...
    await waitFor(() => {
        ...
        expect(mail).toHaveTextContent("ornek@x.com");
